 ...
 * ```
 */
export function getRemixCacheHeaders(config, request, context) {
 ...
 * ```
 */
export function createRemixCacheHeaders(_config) {
 ...
 * ```
 */
export function getCacheHeadersForPath(config, pathname) {
 ...
 * ```
 */
export function withCache(config, loader) {
    return async (args) => {
 ...
 * ```
 */
export function shouldCacheRemixRequest(config, request, context) {
    const url = new URL(request.url);
    const isAuthenticated = context?.isAuthenticated ?? false;
    const isProduction = context?.isProduction ?? process.env.NODE_ENV === 'production';
    const result = evaluateCache(config, {
