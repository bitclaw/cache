...
export const CachePresets = {
    ...
    noCache: {
        ...
        sMaxAge: 0,
        staleWhileRevalidate: 0,
        customDirectives: ['no-cache', 'no-store', 'must-revalidate']
    }
};
/**
 * Helper to create a cache route config with a preset
 */
export function createCacheRoute(pattern, preset, overrides) {
    return {
        pattern,
        ...CachePresets[preset],
        ...overrides
 ...
 */
export function getTTL(cacheControl) {
 ...
 */
export function mergeCacheControl(...headers) {
    if (headers.length === 0)
        return 'no-cache';
    if (headers.length === 1)
        return headers[0];
    ...
    const maxAges = parsed
        .map(p => typeof p['max-age'] === 'string' ? parseInt(p['max-age'], 10) : null)
        .filter((n) => n !== null);
    const minMaxAge = maxAges.length > 0 ? Math.min(...maxAges) : 0;
    // Find minimum s-maxage
    const sMaxAges = parsed
        .map(p => typeof p['s-maxage'] === 'string' ? parseInt(p['s-maxage'], 10) : null)
        .filter((n) => n !== null);
